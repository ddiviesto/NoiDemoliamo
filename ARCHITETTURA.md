...
- **Certificato rottamazione**: 24 ore LAVORATIVE dal ritiro (i weekend non contano: chi ritira venerdì è in regola caricando lunedì)
...
- **Scadenze**: `urgente`, `scadenza_proposta_ritiro` (⚠️ le scadenze dei certificati NON hanno colonne: si calcolano da `data_ritiro_effettuato` con le regole 1.3, cioè 24 ore lavorative per la rottamazione (weekend esclusi) e 15 giorni lavorativi per il PRA)
...
⭐ **Sipario della riassegnazione (12/08)**: il demolitore vede SOLO i messaggi (e conta solo i non letti) da `data_assegnazione` in poi — la chat del predecessore non esiste per lui, su entrambi i canali. Admin e cliente vedono sempre tutto. (Le note e gli eventi condivisi fanno lo stesso via `demolitore_id`.)
...
⭐ **REGOLA DI FATTURAZIONE (più specifico vince)**: fee del veicolo = `pratiche.fee_concordata` se valorizzata (**Trattativa**: importo una tantum per la singola pratica, bypassa tutto; ⭐ 12/08 vale anche **ZERO** = per quel mezzo il demolitore non paga, e la pillola "Trattativa · 0€" lo dice), altrimenti tariffa del COMUNE di ritiro, altrimenti PROVINCIA, altrimenti REGIONE, altrimenti `fee_per_pratica` base.
...
SISTEMA RECENSIONI (da costruire): 2 recensioni obbligatorie prima del certificato
   ↓
Demolitore ha 24 ORE LAVORATIVE per il certificato di rottamazione
   ↓
...
- ⭐ **INDIRIZZO: ripiego a mano (24/08, rifinito 09/09)**. Se i suggerimenti di Google non arrivano (script che non parte, richieste rifiutate, o libreria che non risponde entro 5 secondi), sotto il campo compare una **riga di aiuto grigia** come le altre (non un avviso colorato: non è colpa del cliente) "I suggerimenti non sono disponibili in questo momento: scrivi l'indirizzo completo e premi Continua", e il normale **"Continua"** del passo accetta l'indirizzo scritto (il campo passa al flusso testo e stato via `onTesto`). **Niente secondo bottone "Conferma indirizzo"** nei flussi: resta solo nel campo compatto dell'admin, che non ha un Continua. Con Google acceso, "Continua" senza aver scelto un suggerimento dice "Scegli l'indirizzo tra i suggerimenti". Il campo non si disabilita mai mentre Google carica. Serve a non perdere il cliente, **non è un modo di lavorare**: quella pratica nasce senza comune, provincia e coordinate, quindi l'assegnazione automatica non parte (resta quella a mano) e nel CRM l'indirizzo non è correggibile finché Google è spento (lì si salva solo se scelto dal menu di Google). Quando Google torna, il ripiego sparisce da solo.
...
| **`/admin/valutazioni`** (⭐ 18/09, mockup A) | ✅ | **Gemella di `/admin`** per le richieste di valutazione: barra azzurra, flusso a pillole (Da valutare › Risposta inviata › Passate a demolizione, staccate Rifiutate e Chiuse; stati in `lib/statiValutazione.ts`), righe come le pratiche (veicolo · cliente con intestazione/targhe/fermo · stato con "arrivata 2 ore fa" · risposta data), tendina con 5 schede: **Cronologia e Note** (eventi dalle date + nota admin con la matita) · **Cliente** · **Veicolo** · **Foto** (miniature, clic = apre) · **Risposta al cliente** (bordo celeste: tessere "Non la paghiamo" / "Riconosciamo una cifra", campo che cambia, "Invia la risposta"; dopo l'invio: risposta data, "Cambia la risposta" e "Chiudi senza seguito" con conferma in riga). Il "Cerca targa" a pagamento **non c'è ancora**: da collegare a un servizio di visura (5.5) |
...
- **Fila azioni nella testata**: **Documenti** (contatore + pallino spia, apre direttamente il visore sul primo da verificare) · **Chat** · **Stato pratica** · **Trattativa** (si chiama così, MAI "Extra": copre anche lo zero) · **Assegnazione**, col **cestino** a destra (due scelte: solo la pratica / pratica e account)
- Nella scheda **Ritiro** c'è "**Originali da consegnare al ritiro**": la **LISTA COMPLETA** dei documenti della pratica (⭐ 08/08, IDENTICA in admin, demolitore e pannello Fissa il ritiro, nomi col ruolo casistica): spunta blu = originale da farsi consegnare (`richiede_consegna`, moduli col suffisso ": modulo firmato in originale"), icona fotocamera = documento solo online, pillolina "**Caricato online**" su ciò che il cliente ha già inviato (sta nel visore). Libretto escluso se "da chiarire". Generata dalla checklist: mostra solo ciò che si applica alla pratica
...
- **Tenda "La tua azienda"** (`TendaAzienda.tsx`, mockup approvato 05/08 dopo tre giri: bocciati il pannello da destra, la pagina intera e la tenda sulla colonnina a scomparsa): scivola **da sinistra a destra uscendo dal bordo della barra fissa**; il velo scuro copre SOLO la pagina (la barra resta luminosa e cliccabile). Testata azzurra come le barre del CRM: scrittina "LA TUA AZIENDA" in maiuscoletto e NOME del demolitore protagonista nel grigio dei valori (`#3E4C63`), ✕ a tondino bianco col bordo celeste. 3 schede della famiglia in COLONNA UNICA in sola lettura (Azienda · Sede · Contatti, etichetta a sinistra e valore a destra), indirizzo senza doppioni, dati freschi a OGNI apertura. Si chiude con la ✕, **cliccando sul velo** o RICLICCANDO la voce in sidebar (⭐ 08/08: la voce fa da interruttore, e con la tenda aperta l'evidenziazione "in vetro" della barra passa su "La tua azienda"). Etichetta unica ovunque (tenda, form Nuovo demolitore, scheda CRM): "**Email assegnazioni pratiche**"
- **Home e scheda pratica: FOTOCOPIA del CRM admin** (rifatta da capo il 07/08 su richiesta di Davide: "deve apparire come in admin, senza i poteri"). Barra azzurra con la ricerca a pillola, fila COMPLETA delle caselle-filtro (In arrivo · fissa il ritiro › Ritiro fissato › Certificato rottamazione › Cancellazione targhe › Completate, più "Non a buon fine" fuori fila: bianca a zero, rossa coi casi). **Riga della lista GEMELLA di quella del CRM**: stesse colonne (veicolo 1.6 con targa · marca · anno · km e il comune sotto, cliente 1.3 con la casistica e la delega, stato 1.4 con la pillola e la ragione sociale sotto), hover celeste, spunta verde per le completate, titolo sempre NERO anche da aperta (in entrambe le aree). ⚠️ Le scadenze dei certificati si CALCOLANO da `data_ritiro_effettuato` (24 ore LAVORATIVE rottamazione, weekend esclusi; 15 giorni lavorativi PRA): non hanno colonne nel DB. Il ritardo si mostra in giorni e ore protagonisti ("4g 18h · di ritardo"), mai in sole ore
- **Tendina della pratica** (`TendinaPratica.tsx`): clic sulla riga → si srotola sotto col blocco unico e la cornice blu, IDENTICA al CRM (riclic/Esc chiude, cambiare casella chiude). Fila azioni a pillole sulla coda azzurra della testata, in QUEST'ORDINE: "**Documenti e Foto**" col contatore (apre il visore condiviso) · "**Chat**" con la spia rossa · l'**azione della fase** ("Fissa il ritiro" · "Veicolo ritirato" e "Sposta il ritiro" col motivo obbligatorio · "Carica certificato rottamazione" e "Consegnato a mano" · "Carica cancellazione targhe") · la **pillola "Trattativa · N€"** quando c'è (anche "· 0€"). Le **5 schede gemelle di quelle del CRM** (Cronologia e Note · Cliente · Casistiche · Veicolo · Ritiro), stesse misure, TUTTE ALLA STESSA ALTEZZA, in sola lettura: il "Chiama" va sul DELEGATO quando c'è, la scheda Ritiro ha gli "Originali da consegnare" con le spunte blu. **Precarico all'hover**: dettagli E cronologia si scaricano al passaggio del mouse sulla riga (cache), la tendina si apre già piena senza sobbalzi. Le **nuvolette** della fila vivono in un PORTALE sul body (il sipario dell'animazione non le taglia; su schermi bassi scorrono al loro interno)
- ⭐ **"Fissa il ritiro" e "Sposta il ritiro" = PANNELLO DA DESTRA** (come Documenti e Foto, approvato sul vero 08/08 dopo tre giri di mockup): velo leggero, entrata/uscita animate, corpo **grigio area-lavoro** con le card bianche sopra. Testata azzurra: titolo blu, targa · veicolo · indirizzo e la pillola del **VIAGGIO** "N km · circa N min" (SENZA icona; Distance Matrix dal server con la cache per pratica). Dentro (`PickerRitiro` in modalità `pannello`): **calendario PROTAGONISTA sempre aperto** con le celle che si SCALDANO di celeste col carico (variante C: "fa vedere i buchi"), i numerini degli impegni, la legendina, 6 righe fisse, e RESTA APERTO sfogliando; **ORE a SLOT PARLANTI sottili** in due colonne Mattina | Pomeriggio: gli occupati sono sbarrati e DICONO cosa c'è ("09:00 CK456ZY · Milano"), lo scelto è blu con "il tuo nuovo ritiro", le passate spente. Colonna destra: "**La tua giornata**" (card con testata, contatore, stati vuoti disegnati, il NUOVO al posto giusto) + card "**Informazioni per il ritiro**" (Chiama sul delegato, condizioni a pilloline, spazio carro, casistica, originali). Piede a barra: nota "Chiama prima il cliente per accordarvi: vedrà la data nella sua area" (MAI "la data vale subito") oppure il riepilogo blu; Conferma si sblocca a scelta completa; per Sposta c'è la sezione del motivo obbligatorio. L'agenda conta pratiche E impegni personali; la nuvoletta "**Aggiungi impegno**" dei Ritiri usa lo stesso picker in versione pillole (calendarietto a scomparsa) con la stessa veste (testata azzurra + piede a barra)
- **Visore documenti CONDIVISO** (`app/components/VisoreDocumenti.tsx`): "Documenti e Foto" apre LO STESSO pannello del CRM (scivola da destra, testata azzurra, elenco con miniature sulla **colonnina grigia area-lavoro**, palco ardesia su area BIANCA con zoom, PDF sfogliabili, frecce a rotazione, Scarica col PDF unico) in **SOLA LETTURA** (niente Approva/Rifiuta), coi nomi dei documenti col RUOLO della casistica. I pezzi (`ZoomImg`, `PdfZoom`, `nomeAdmin`, `scaricaPdfVoci`) vivono nel componente condiviso e li usa ANCHE il visore admin: i due restano gemelli per sempre. Dati dagli endpoint demolitore (URL sempre rifirmati dal server: il bucket è privato)
...
- **`/api/admin-valutazioni`** (⭐ 18/09) — le richieste di valutazione per l'admin, col service role perché `veicoli_vendita` è visibile solo al proprietario. Azioni: `lista` (con le foto), `rispondi` (tipo `demolizione` = "Non la paghiamo" col motivo obbligatorio, `acquisto` = "Riconosciamo X €" con importo; mette `risposta_inviata` e qui andrà l'email "risposta pronta"), `chiudi` (senza seguito), `riapri` (torna "da valutare", anche per cambiare la risposta), `nota` (`note_admin`). Una richiesta già `passata_demolizione` non si tocca più.
...
- **`/api/pratica-fee`** — Trattativa: imposta o rimuove l'importo concordato (0 compreso).
...
- **Area demolitore** (il ruolo demolitore non tocca MAI il DB direttamente, RLS): `/api/demolitore-pratiche` (il dettaglio RIFIRMA sempre gli URL dei documenti del bucket privato, e manda `documenti_ritiro` — lista completa consegna/caricato — e `viaggio` km · minuti via Distance Matrix con la cache in memoria per pratica), `/api/demolitore-azioni` (fissa/sposta ritiro — **spostare richiede il motivo, il server rifiuta senza** —, segna ritirata, rottamazione a mano), `/api/demolitore-certificato`, `/api/demolitore-chat`, `/api/demolitore-rapidi` (frasi rapide SUE della chat, semina + gestione), `/api/demolitore-impegni` (impegni personali della pagina Ritiri), `/api/demolitore-note`, `/api/demolitore-profilo`. Auth condivisa in `lib/demolitoreAuth.ts`.
...
3b. ⭐ **LA SCENA "LA 500 VA DOVE SCEGLI" (25/08, mockup approvato)**, sopra le due porte e **solo da PC** (sotto i 640px non c'è: le porte si incolonnano). Al centro la **500 rossa** ferma, a sinistra il **carro attrezzi** col pianale abbassato e la rampa rivolta verso di lei, a destra **chi la compra** coi contanti in mano. Le due mete stanno **smorte e in disparte** finché non si sceglie: la protagonista è la 500.
   **La manovra si fa per davvero, niente salti**: passando il mouse sulla porta della demolizione la 500 *si gira*, imbocca la rampa e ci sale; passando sull'altra *scende in retromarcia* fino al centro, *si gira* e va dall'acquirente, fermandosi a un dito dai contanti.
   ⚠️ **Si accende col mouse sopra la porta, MAI al clic**: il clic deve aprire subito il modulo e la manovra dura più di due secondi. Chi ha attivato "riduci il movimento" vede tutto fermo.
   Codice in `app/components/ScenaHome.tsx` (le porte si presentano con `data-meta`), veste in `globals.css` (`.scena-home`). Il punto d'arrivo **non è scritto a mano**: si calcola misurando dove sono davvero il pianale (una riga inclinata di 17,4° dentro il disegno) e l'acquirente, così resta preciso a ogni larghezza.
   **Le illustrazioni sono di Davide** (generate con Gemini, stile piatto senza contorni), ripulite dallo sfondo, ritagliate strette e **messe in scala vera** fra loro: il carro è lungo il doppio della 500, la persona le arriva sopra il tetto. Stanno in `public/`: `auto-500.png` · `carro-attrezzi.png` · `acquirente.png`.
   ⚠️ **Misure in percentuale, mai in pixel** (le proporzioni reggono a ogni larghezza), **niente riga di terra** sotto i mezzi (tagliava le ruote: a tenere giù la 500 basta la sua ombra) e la **500 sta appena a destra del centro**, se no si appoggia sulla punta della rampa e i tre sembrano ammucchiati.
...
- **Targa e codice fiscale**: stessa pillola in versione `--grande` (19px, centrata, spaziata), si leggono come su una targa vera. ⭐ 09/09 (mockup A): la targa è anche **corta** (`--targa`, larga quanto una targa, allineata a sinistra) e la domanda **"Le targhe sono sul mezzo?" è un campo come gli altri**: etichetta sopra, due pillole uguali (`.scelte-griglia--due`), niente riquadro azzurro intorno. Se "smarrite", sotto compare la nota azzurra (`InfoBadge`): in demolizione "denuncia di smarrimento in originale, da consegnare al ritiro"; in valutazione **senza nominare la demolizione**: "Nessun problema: servirà solo una denuncia di smarrimento. Con la burocrazia ti aiutiamo noi."
...
2. ~~Voce "Valutazioni" nella sidebar admin~~ **FATTA (18/09)**: `/admin/valutazioni`, vedi 5.3. Manca solo il bottone **"Cerca targa"** a pagamento (visura ~6 €, vedi 5.5): da collegare quando si sceglie il servizio
...
- ⭐ **Fee del demolitore per ZONA con fatturazione automatica** (regola più-specifico-vince, vedi 3.10). La **Trattativa** bypassa ogni tariffa per la singola pratica (anche a ZERO: mezzo gratis per il demolitore) e finisce in **proforma fattura** così com'è
