...
**Seconda parte (28/07 sera) — ⭐⭐⭐ DEMOLITORI COME LE PRATICHE: tutto nella tendina (mockup approvato, regola di Davide: NON si perde niente, cambia solo l'aspetto):**
- ⭐⭐⭐ **Pagina di dettaglio `/admin/demolitori/[id]` ELIMINATA**: la scheda vive nella TENDINA sotto la riga (`TendinaDemolitore.tsx`; `MappaComuni` resta nella cartella `[id]/`, importata). Clic sulla riga = apre, riclic/Esc = chiude.
- ⭐⭐ **Testata aperta = la STESSA riga della card chiusa** (stesse misure, colonne e barretta di stato: NIENTE sobbalzo), si tinge d'azzurro col nome in blu. **I 4 QUADRATINI (Aperte · Completate · Annullate · Velocità) si vedono anche a scheda CHIUSA** (conteggi calcolati per tutti nella lista e passati alla tendina: numeri giusti da subito); Annullate rossa tenue e cliccabile → **pannello da destra coi motivi** (clic sulla pratica → `/admin?apri=<id>`).
- ⭐⭐ **Fila azioni a pilloline**: **Accesso area** (LED verde/rosso; dentro la nuvoletta: Reinvita blu + **Revoca a pillola bianca testo rosso spento** (variante A su mockup, via il link rosso), conferma inline, link di riserva con Copia quando l'email non è configurata), **Stato** (nuvoletta con le pillole vere; ⭐ cambiando stato la tendina si RICHIUDE animata e la riga si sposta tra attivi/non attivi SENZA riaprirsi: il "ti segue" è solo delle pratiche), **Copertura** (senza numerino) → **PANNELLO DA DESTRA** con chips zone (badge "parziale" incluso) e la mappa vera (⭐ il bottone "Cancella tutto" è diventato l'icona cestino), cestino eliminazione con **conferma scrivendo il NOME** (blocco server su pratiche aperte invariato).
- ⭐⭐ **Schede con la matita** (modifica sul posto, campi filo blu): **Cronologia e Note** (aggiungi con Invio, elimina al passaggio), **Azienda** (con **autocomplete Google** della sede), **Contatti**, **Persone** (titoletti Titolare/Referente in ardesia), **Tariffa** (rinominata da Contribuzione: ⭐ la BASE è una RIGA come le zone — variante A, via il rettangolone celeste; tariffe con badge zona e "Fuori copertura"; "**+ Aggiungi zona**" apre MORBIDA la sotto-area celeste con pilloline tipo, suggerimenti e tondo blu; via la frase della regola).
...
**Terza parte (28/07 notte) — ⭐⭐⭐ GIRO MOBILE DAGLI SCREEN IPHONE (Davide contentissimo del risultato):**
- ⭐⭐ **Via il NERO attorno alla pagina** (bug trovato da Davide): il TEMA SCURO del telefono anneriva lo sfondo dietro le pagine (regola dark-mode nel CSS) — il sito è dichiarato SOLO CHIARO, `html` lavanda, e l'elastico di iOS mostra lavanda.
- ⭐ **Campo chat leggibile**: placeholder più scuro, bordo presente e **testo 16px sul telefono** (sotto i 16px Safari zooma al tocco — regola già nota, ora applicata anche qui).
- ⭐⭐⭐ **APP A TUTTO SCHERMO su mobile** (mockup approvato, riferimento: siti assicurativi full-bleed): home cliente e pagina pratica senza cornice lavanda — bianco fino ai bordi, header blu in cima che si FONDE con la cornice del browser (`theme-color #1d4ed8`). Su PC resta la card centrata di sempre (`sm:`).
- ⭐⭐⭐ **VISORE CLIENTE A PALCO SCURO** (mockup approvato): i file caricati e le foto si aprono a tutto schermo sul grigio ardesia dell'admin, documento centrato con l'ombra, titolo + ✕, "**‹ Prec. · x di N · Succ. ›**" tra i file dello stesso documento o tra le foto (la lista si ricostruisce dal file aperto in `apriAnteprima`); PDF a pagina intera. ⚠️ Lo zoom col pizzico è bloccato dal viewport anti-zoom del flusso: se servirà, controllo di zoom dedicato nel visore.
- ⚠️ **Test dal telefono via rete locale: PROVATO E ABBANDONATO** (regola firewall + rete privata poi rimosse): il dev server serve JS non transpilato che BLOCCA Safari iPhone (pagina visibile, tocchi morti). Metodo definitivo: **verifiche da PC → push → iPhone su Vercel → screen**. Se mai servisse il telefono senza push: `npm run build` + `npm run start` (produzione locale), MAI il dev server.
- ✅ ~~PROSSIMA SESSIONE (mobile)~~: FATTO nella quarta parte (sotto). Nota Android: stesso codice per tutti i telefoni, differenze solo nelle finezze dei browser — conferma finale con lo STEP 7 (LambdaTest su Android veri).

**Quarta parte (28/07 sera/notte) — ⭐⭐⭐ SESSIONE AL SIMULATORE (nuovo metodo) + RAFFICA DI RIFINITURE + CRONOLOGIA A DUE CANALI:**
- ⭐⭐⭐ **NUOVO METODO DI LAVORO: il SIMULATORE di Chrome** (F12 → Ctrl+Shift+M → iPhone, "Show device frame"): Davide vede ogni modifica all'istante su localhost SENZA push (il mouse fa da dito, i gesti touch funzionano). Push solo a fine raffica; l'iPhone vero su Vercel resta il collaudo finale (le finezze Safari si vedono solo lì). Nota: se il simulatore è zoomato sotto il 100% i testi sembrano sfocati — è lo zoom, non l'app.
- ⭐⭐ **/inizia A TUTTO SCHERMO sul telefono** (mockup approvato): via la cornice lavanda, banner blu del passo fuso col browser; su PC identico a prima. Un solo `main`, cambio di cornice per tutti i 14-15 passi.
- ⭐ **/inizia rifiniture**: riquadri dell'intestazione TUTTI alla stessa altezza (griglia `gridAutoRows 1fr` + minimo 92px — il testo lungo non fa più spiccare un riquadro); etichetta accorciata "**Passaggio di proprietà non completato**"; avviso del fermo = **scheda blu** di famiglia (via il giallo), titolo "Il debito non si cancella", testi più grandi.
- ⭐⭐ **DOCUMENTO RIFIUTATO ripensato** (mockup 4 scene): il rifiuto **AZZERA i file** del cliente (riga svuotata + pulizia bucket best-effort) — ritrova il documento come alla prima vista col motivo nella testata rossa; banner cliente in **rosso TENUE** ("Qui sotto vedi quali sono e cosa correggere", via il gergo "tab"); frasi che seguono il modo (foto: "Rifai le due foto: quelle vecchie le abbiamo tolte noi" · file: "Allega il documento nuovo"); bottone "Invia di nuovo". **Allega file = pillola compatta** centrata (via il rettangolone tratteggiato); i file allegati sono **miniature 56px come le foto** (badge PDF, ✕ sull'angolo, nome piccolo sotto).
- ⭐⭐⭐ **CRONOLOGIA A DUE CANALI + PERCORSO COMPLETO** (mockup approvati, punto importante per Davide — SQL `docs/sql/2026-07-28-cronologia-percorso.sql` ESEGUITO: colonne `evento`, `visibile_demolitore`, `demolitore_id` su `pratiche_note`):
  - Canale **NoiDemoliamo = REGISTRO COMPLETO** (eventi automatici + note private + note del quadernetto col demolitore, firmate); pillolina **Demolitore = solo FILTRO** della vista di lui, e da lì gli si scrive ("quello che scrivi qui lo vede il demolitore").
  - **Eventi automatici** con pilloline PARLANTI: cambio stato (pillola ESATTA del CRM — `lib/statiCrm.ts`, nato spostando STATO_META da admin/page), documento rifiutato (solo nome documento a capo, SENZA motivo), documenti approvati, assegnata/riassegnata/disassegnata, ritiro fissato/spostato, ritirata, certificati, trattativa (solo admin). Agganciati in: pratica-stato, assegna-pratica, demolitore-azioni, demolitore-certificato, pratica-fee, DocumentiApprovazione.
  - **Ripresa** dall'attesa: accanto alla pillola c'è la pillola di stato CRM di dove riparte.
  - **Riassegnazione**: il nuovo demolitore riparte da zero (voci legate a `demolitore_id`), al vecchio sparisce la pratica, la storia resta tutta all'admin. `/api/demolitore-note` serve il canale condiviso; componente `NoteDemolitore` (parcheggiato) aggiornato. 🔜 Più avanti: rifinire cosa il demolitore NON deve vedere quando nasceranno nuovi stati.
- ⭐ **CRM rifiniture**: pillole ROSSE della riga sempre rosse (non sbiancano più da aperta/hover); **km dopo l'anno** in testata riga; "Delegato per la consegna"; **pillolina celeste** sui 4 dati operativi (delegato+tel, fermo, targhe, spazio carro — mockup "Pillola 1"); blocchetti celesti "NOTA CLIENTE SUL VEICOLO/SULLO SPAZIO" (prima le note libere del cliente non si vedevano DA NESSUNA PARTE in admin — `note_veicolo` e `spazio_carro_attrezzi_note` aggiunte a CAMPI_LISTA); **indirizzo ritiro in riga unica a due righe** (via+civico, poi CAP e comune; via la riga Comune doppione — l'indirizzo di Google li contiene già, stesso fix nella scheda Azienda del demolitore); **Email del cliente via `/api/email-cliente`** (il browser admin non può leggere `utenti` per RLS: mostrava "—" — ora email di login vera via server).
- ⭐ **Delegato modificabile dal CLIENTE** (mockup A): nella card "Dati del veicolo" della tab Stato, righe delegato/telefono col link "Modifica" (una alla volta, campo filo blu 16px) — SOLO pre-assegnazione, via `/api/pratica-dati` in modalità cliente (solo quei 2 campi), riflesso subito nel CRM. Il delegato è DELLA PRATICA (le Impostazioni restano il telefono dell'account: pratiche diverse = delegati diversi, nessun conflitto).
- ⭐ **Pallino rosso FISSO sulla linguetta Ritiro** del cliente: dalla nascita della pratica fino al ritiro effettivo (o annullo) — aprire la tab non lo spegne più (via la memoria "data vista").
- Altre rifiniture cliente: card pratica in home (mix B+C: ombra + targa 16.5 + barretta blu + scritte taglia 2, tipo e modello su DUE righe, "1 pratica attiva" a 13); tab Ritiro coi **riquadrini celesti** per documento; nomi documenti inviati 13.5 peso 500; via la scritta "Toccalo per vederlo…" dal retro del pannello inviati; "Delegato per la consegna" anche nella card Dati del veicolo.
...
- ⭐⭐ **DATE DI RITIRO IN RIGA** (variante 2 su mockup, boxini a destra): quando il demolitore le imposta si vedono anche nel CRM — "**Ritiro Programmato**" (grigio-azzurro quieto, `data_ritiro_prevista`) e "**Ritiro Effettivo**" (azzurro, `data_ritiro_effettuato`), affiancati con la data gg/mm/aa CON L'ANNO; senza date resta il countdown rosso. Annullate: nessun boxino.
- ⭐ **RINOMINE STATI** (Davide, con ripensamento finale): `ritiro_confermato` = "**Ritiro Programmato**" (secco, senza "Assegnata ·"); la famiglia del ritiro resta "**Ritirata**" ("Mezzo Ritirato" provato e ripensato): "Ritirata · Attesa Certificati" (ritirata e attesa rottamazione), "Ritirata · attesa recensione", "Ritirata · Attesa PRA". Coerente con la casella "Ritirata" del flusso.
- ⭐⭐ **FLUSSO ESTESO** (variante B su mockup): dopo Assegnata/Allerta entra la casella "**Ritiro Programmato**" (bucket `programmato` = stato ritiro_confermato) e poi la **colonnina "Ritirata"** in stile Assegnata/Allerta: "Ritirata" è la SOMMA e sotto "**Attesa Certificati**" (ritirata + attesa rottamazione + recensione) e "**Attesa PRA**" (attesa radiazione) spezzano il conto; ogni pillola è anche filtro ("Ritirata" = macro-filtro delle due). NIENTE casella recensioni: si gestiranno in automatico con email e link. (Davide: "per ora la B, in futuro miglioriamo".)
- ⭐ **UN SOLO AZZURRO `#EFF6FF`** (mockup approvato): la barra "Pratiche"/"Demolitori" in alto è AZZURRA come la testata della pratica aperta (bordo sotto `#DBEAFE`, ricerca a pillola BIANCA col bordo celeste) e le righe della lista al passaggio del mouse si tingono dello stesso azzurro con bordo `#BFDBFE` — barra, hover e apertura parlano lo stesso colore. Vale anche per le card dei DEMOLITORI (hover gemello; la barretta colorata dello stato a sinistra resta com'è).
...
- ⭐ **FOTO SENZA DESCRIZIONI** (richieste Davide): nell'ELENCO del visore le foto del veicolo sono **solo miniature in griglia** (quadratini 48px, anello blu sull'attiva, casellina sovrapposta in selezione — via le righe "Foto del veicolo N"); nel **PDF** le pagine delle foto vanno **pulite senza etichetta** (le etichette restano solo sui documenti) e se il PDF contiene **solo foto** si chiama "**Foto TARGA.pdf**" invece di "Documenti".
- ⭐⭐ **ASSEGNAZIONE = PANNELLO DA DESTRA** (variante B su mockup, dopo 2 giri bocciati: a destra delle schede le strizzava, a tutta larghezza era "anti professionale"): gemello del visore Documenti — scivola da destra (~470px, stessa animazione), **testata azzurra** "Assegnazione · TARGA" col sottotitolo di contesto (quanti coprono la zona / assegnata a chi / prima approva i documenti), ✕/clic fuori/Esc chiudono, la tendina sotto NON si muove. Dentro: bottoni "Assegna in automatico" e "Scegli tu", **candidati a SCHEDINE** (bordo proprio, consigliato con bordo celeste e Assegna blu pieno; carico "N da ritirare" nel testo informativo), lista che scorre a tutta altezza; assegnata = box demolitore + Riassegna/Rimuovi con conferma in linea. La pillola Documenti chiude il pannello assegnazione (mai due pannelli sovrapposti).
- ⭐⭐ **ANIMAZIONI DI FAMIGLIA "DA DESTRA"** (richieste Davide): i pannelli Documenti e Assegnazione **entrano scivolando da destra e escono riscivolando verso destra** prima di smontarsi (clic fuori, ✕, Esc e anche RICLIC sulla pillola, che fa da interruttore — pattern `chiudiSegnale` incrementale + ref, e stato `chiudendo` con transizione 240ms). Anche la **CHAT** (variante B su mockup animato, scelta contro "sale dal basso" e "sboccia"): la finestrella scivola da destra all'apertura e via verso destra alla chiusura (✕ o riclic della pillola), stessa famiglia di movimento dei pannelli.
