**1)DEMOLIZIONE MEZZO INTESTATO A PERSONA FISICA:**

**Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo (può fare foto direttamente dal telefono o caricamento file):**

* Libretto Circolazione (fronte e retro)
* Denuncia di smarrimento Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo (fronte e retro) (Se è Digitale non deve caricarlo e non serve)
* Denuncia di smarrimento Certificato di Proprietà Cartaceo (se smarrito)
* Carta Identità **Proprietario mezzo** (fronte e retro) o Patente di guida **Proprietario mezzo** (fronte e retro) se Carta Identità **Proprietario mezzo** non disponibile
* Codice Fiscale **Proprietario mezzo** (fronte e retro)
* Denuncia di smarrimento Carta Identità **Proprietario mezzo** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Proprietario mezzo** (se smarrito)



Se **Proprietario mezzo** delega un'altra persona alla consegna del mezzo al Demolitore:

* Carta Identità **Delegato** (fronte e retro) o Patente di guida **Delegato** (fronte e retro) se Carta Identità **Delegato** non disponibile
* Codice Fiscale (fronte e retro) **Delegato**
* Denuncia di smarrimento Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Delegato** (se smarrito)



**Documenti PDF da stampare e firmare prima del ritiro:**

* **DELEGA\_CONSEGNA\_VEICOLO\_PRIVATO** (auto compilata dal sistema) se **Proprietario mezzo** delega un'altra persona alla consegna del mezzo al Demolitore



**Documenti da consegnare al Demolitore al ritiro (nella pagina del cliente ci dovrà essere una sezione riepilogativa. Dovrà ricevere anche un sms, email e notifica APP come promemoria):**

* Libretto di Circolazione originale
* Denuncia di smarrimento originale Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo originale (se è Digitale no)
* Denuncia di smarrimento originale Certificato di Proprietà Cartaceo (se smarrito)
* Denuncia di smarrimento originale Carta Identità **Proprietario mezzo** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Proprietario mezzo** (se smarrito)
* **DELEGA\_CONSEGNA\_VEICOLO\_PRIVATO** in originale compilata e firmata dal **Delegante** (**Proprietario mezzo**) e **Delegato** (Se si delega qualcun altro)
* Denuncia di smarrimento originale Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Delegato** (se smarrito)







**2)DEMOLIZIONE MEZZO PROPRIETARIO DECEDUTO (EREDI HANNO ACCETTATO EREDITÀ)**

**Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo (può fare foto direttamente dal telefono o caricamento file):**

* **Atto di Morte proprietario deceduto**
* Libretto Circolazione (fronte e retro)
* Denuncia di smarrimento Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo (fronte e retro) (Se è Digitale non deve caricarlo e non serve)
* Denuncia di smarrimento Certificato di Proprietà Cartaceo (se smarrito)
* Identificare il **numero di eredi (il sistema deve dare più slot di caricamento in base alla selezione**:
* Carta Identità **Erede o Eredi** (fronte e retro) o Patente di guida **Erede o Eredi** (fronte e retro) se Carta Identità **Erede o Eredi** non disponibile
* Codice Fiscale **Erede o Eredi** (fronte e retro)
* Denuncia di smarrimento Carta Identità **Erede o Eredi** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Erede o Eredi** (se smarrito)



Se **Erede o Eredi** delega un'altra persona alla consegna del mezzo al Demolitore:

* Carta Identità **Delegato** (fronte e retro) o Patente di guida **Delegato** (fronte e retro) se Carta Identità **Delegato** non disponibile
* Codice Fiscale (fronte e retro) **Delegato**
* Denuncia di smarrimento Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Delegato** (se smarrito)



**Documenti PDF da stampare e firmare prima del ritiro:**

* **DELEGA CONSEGNA VEICOLO\_EREDI** (auto compilata dal sistema) se **Erede o Eredi** **(delega/delegano)** un'altra persona alla consegna del mezzo al Demolitore
* **DICHIARAZIONE\_SOSTITUTIVA\_EREDITÀ** (auto compilata dal sistema)



**Documenti da consegnare al Demolitore al ritiro (nella pagina del cliente ci dovrà essere una sezione riepilogativa. Dovrà ricevere anche un sms, email e notifica APP come promemoria):**

* Libretto di Circolazione originale
* Denuncia di smarrimento originale Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo originale (se è Digitale no)
* Denuncia di smarrimento originale Certificato di Proprietà Cartaceo (se smarrito)
* Denuncia di smarrimento originale Carta Identità **Erede o Eredi** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Erede o Eredi** (se smarrito)
* **DELEGA CONSEGNA VEICOLO\_EREDI** in originale compilata e firmata dal **Erede o Eredi** e **Delegato** (Se si delega qualcun altro)
* Denuncia di smarrimento originale Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Delegato** (se smarrito)
* **DICHIARAZIONE\_SOSTITUTIVA\_EREDITÀ** in originale compilata e firmata
* **COPIA/FOTOCOPIA ATTO DI MORTE PROPRIETARIO DECEDUTO**







**3)DEMOLIZIONE MEZZO PROPRIETARIO DECEDUTO (ALCUNI EREDI HANNO RINUNCIATO, ALMENO UNO HA ACCETTATO)**

NOTE PER L'UTENTE (Visualizzate nella piattaforma)

**Quando usare questa opzione:** Seleziona questa casistica solo se il proprietario del veicolo è deceduto e tra i parenti aventi diritto (es. coniuge, figli, fratelli, nipoti, cugini o altri parenti legittimi) qualcuno ha rinunciato formalmente (tramite Notaio o Tribunale), mentre almeno uno di questi stessi parenti ha accettato l'eredità e si prende l'incarico di demolire il mezzo.

**Attenzione:** Chi ha rinunciato all'eredità NON deve firmare nulla e NON deve consegnare i propri documenti, altrimenti rischia di annullare la rinuncia. La pratica verrà firmata e gestita solo da chi ha accettato, che dichiarerà la rinuncia degli altri nell'autocertificazione generata dall'app.



NOTE PER IL SISTEMA (Logica di Backend / Database)

Condizione di sblocco (Trigger): Questa casistica si attiva se l'utente risponde "SÌ" alla domanda "Ci sono eredi che hanno rinunciato all'eredità?" AND "SÌ" alla domanda "C'è almeno un erede che ha accettato?".



Contatore Slot Dinamici: Il sistema deve generare gli slot di upload dei documenti d'identità esclusivamente per il numero di eredi dichiarati come "Accettanti". Non richiedere dati anagrafici o documenti d'identità per i rinunciatari.



Mappatura PDF: I nomi dei parenti rinunciatari (inseriti dall'utente in un semplice campo di testo) devono essere iniettati automaticamente nel PDF della **DICHIARAZIONE\_SOSTITUTIVA\_EREDITÀ\_RINUNCIA** nella sezione "Dichiara inoltre che i signori X, Y, Z hanno rinunciato al diritto di successione".





**Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo (può fare foto direttamente dal telefono o caricamento file):**

* **Atto di Morte proprietario deceduto**
* Libretto Circolazione (fronte e retro)
* Denuncia di smarrimento Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo (fronte e retro) (Se è Digitale non deve caricarlo e non serve)
* Denuncia di smarrimento Certificato di Proprietà Cartaceo (se smarrito)
* **Identificare il numero di eredi CHE HANNO ACCETTATO L'EREDITÀ (il sistema dà più slot di caricamento in base alla selezione):**
* Carta Identità **Erede o Eredi** (fronte e retro) o Patente di guida **Erede o Eredi** (fronte e retro) se Carta Identità **Erede o Eredi** non disponibile
* Codice Fiscale **Erede o Eredi** (fronte e retro)
* Denuncia di smarrimento Carta Identità **Erede o Eredi** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Erede o Eredi** (se smarrito)



Se **Erede o Eredi** delega un'altra persona alla consegna del mezzo al Demolitore:

* Carta Identità **Delegato** (fronte e retro) o Patente di guida **Delegato** (fronte e retro) se Carta Identità **Delegato** non disponibile
* Codice Fiscale (fronte e retro) **Delegato**
* Denuncia di smarrimento Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Delegato** (se smarrito)



**Documenti PDF da stampare e firmare prima del ritiro:**

* **DELEGA CONSEGNA VEICOLO\_EREDI\_RINUNCIA** (auto compilata dal sistema) se **Erede o Eredi** **(delega/delegano)** un'altra persona alla consegna del mezzo al Demolitore
* **DICHIARAZIONE\_SOSTITUTIVA\_EREDITÀ\_RINUNCIA** (auto compilata dal sistema)



**Documenti da consegnare al Demolitore al ritiro (nella pagina del cliente ci dovrà essere una sezione riepilogativa. Dovrà ricevere anche un sms, email e notifica APP come promemoria):**

* Libretto di Circolazione originale
* Denuncia di smarrimento originale Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo originale (se è Digitale no)
* Denuncia di smarrimento originale Certificato di Proprietà Cartaceo (se smarrito)
* Denuncia di smarrimento originale Carta Identità **Erede o Eredi** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Erede o Eredi** (se smarrito)
* **DELEGA CONSEGNA VEICOLO\_EREDI\_RINUNCIA** in originale compilata e firmata dal **Erede o Eredi** e **Delegato** (Se si delega qualcun altro)
* Denuncia di smarrimento originale Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Delegato** (se smarrito)
* **DICHIARAZIONE\_SOSTITUTIVA\_EREDITÀ\_RINUNCIA** in originale compilata e firmata
* **COPIA/FOTOCOPIA ATTO DI MORTE PROPRIETARIO DECEDUTO**











**4)DEMOLIZIONE MEZZO INTESTATO A SOCIETÀ/AZIENDA**

**Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo (può fare foto direttamente dal telefono o caricamento file):**

* **Visura Camerale Società/Azienda** (dove si vedono i dati del **Legale Rappresentante** di solito è la prima pagina della visura)
* Libretto Circolazione (fronte e retro)
* Denuncia di smarrimento Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo (fronte e retro) (Se è Digitale non deve caricarlo e non serve)
* Denuncia di smarrimento Certificato di Proprietà Cartaceo (se smarrito)
* Carta Identità **Legale Rappresentante Società/Azienda** (fronte e retro) o Patente di guida **Legale Rappresentante Società/Azienda** (fronte e retro) se Carta Identità **Legale Rappresentante Società/Azienda** non disponibile
* Codice Fiscale **Legale Rappresentante Società/Azienda** (fronte e retro)
* Denuncia di smarrimento Carta Identità **Legale Rappresentante Società/Azienda** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Legale Rappresentante Società/Azienda** (se smarrito)



Se **Legale Rappresentante Società/Azienda** delega un'altra persona alla consegna del mezzo al Demolitore:

* Carta Identità **Delegato** (fronte e retro) o Patente di guida **Delegato** (fronte e retro) se Carta Identità **Delegato** non disponibile
* Codice Fiscale (fronte e retro) **Delegato**
* Denuncia di smarrimento Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Delegato** (se smarrito)



**Documenti PDF da stampare e firmare prima del ritiro:**

* **DELEGA\_CONSEGNA\_VEICOLO\_SOCIETÀ\_AZIENDA** (auto compilata dal sistema) se **Legale Rappresentante Società/Azienda** delega un'altra persona alla consegna del mezzo al Demolitore
* **DICHIARAZIONE\_SOSTITUTIVA\_LEGALE\_RAPPRESENTANTE** (auto compilata dal sistema)



**Documenti da consegnare al Demolitore al ritiro (nella pagina del cliente ci dovrà essere una sezione riepilogativa. Dovrà ricevere anche un sms, email e notifica APP come promemoria):**

* Libretto di Circolazione originale
* Denuncia di smarrimento originale Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo originale (se è Digitale no)
* Denuncia di smarrimento originale Certificato di Proprietà Cartaceo (se smarrito)
* Denuncia di smarrimento originale Carta Identità **Legale Rappresentante Società/Azienda** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Legale Rappresentante Società/Azienda** (se smarrito)
* **DELEGA\_CONSEGNA\_VEICOLO\_SOCIETÀ\_AZIENDA** in originale compilata e firmata dal **Delegante** (**Legale Rappresentante Società/Azienda**) e **Delegato** (Se si delega qualcun altro)
* Denuncia di smarrimento originale Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Delegato** (se smarrito)
* **DICHIARAZIONE\_SOSTITUTIVA\_LEGALE\_RAPPRESENTANTE** in originale compilata e firmata









**5)DEMOLIZIONE MEZZO INTESTATO A SOCIETÀ/AZIENDA (FALLITA):**

**Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo (può fare foto direttamente dal telefono o caricamento file):**

* **Autorizzazione del Giudice Delegato alla demolizione del mezzo**
* **Visura Camerale Storica o per Procedure Concorsuali dell'Azienda** (dove si evince lo stato di fallimento e la nomina del Curatore Fallimentare)
* Libretto Circolazione (fronte e retro)
* Denuncia di smarrimento Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo (fronte e retro) (Se è Digitale non deve caricarlo e non serve)
* Denuncia di smarrimento Certificato di Proprietà Cartaceo (se smarrito)
* Carta Identità **Curatore Fallimentare** (fronte e retro) o Patente di guida **Curatore Fallimentare** (fronte e retro) se Carta Identità **Curatore Fallimentare** non disponibile
* Codice Fiscale **Curatore Fallimentare** (fronte e retro)
* Denuncia di smarrimento Carta Identità **Curatore Fallimentare** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Curatore Fallimentare** (se smarrito)



Se **Curatore Fallimentare** delega un'altra persona alla consegna del mezzo al Demolitore:

* Carta Identità **Delegato** (fronte e retro) o Patente di guida **Delegato** (fronte e retro) se Carta Identità **Delegato** non disponibile
* Codice Fiscale (fronte e retro) **Delegato**
* Denuncia di smarrimento Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Delegato** (se smarrito)



**Documenti PDF da stampare e firmare prima del ritiro:**

* **DELEGA\_CONSEGNA\_VEICOLO\_FALLIMENTO** (auto compilata dal sistema) se **Curatore Fallimentare** delega un'altra persona alla consegna del mezzo al Demolitore
* **DICHIARAZIONE\_SOSTITUTIVA\_CURATORE\_FALLIMENTARE** (auto compilata dal sistema)



**Documenti da consegnare al Demolitore al ritiro (nella pagina del cliente ci dovrà essere una sezione riepilogativa. Dovrà ricevere anche un sms, email e notifica APP come promemoria):**

* **Autorizzazione del Giudice Delegato alla demolizione del mezzo** (in originale)
* **Visura Camerale Storica o per Procedure Concorsuali dell'Azienda**
* Libretto di Circolazione originale
* Denuncia di smarrimento originale Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo originale (se è Digitale no)
* Denuncia di smarrimento originale Certificato di Proprietà Cartaceo (se smarrito)
* Denuncia di smarrimento originale Carta Identità **Curatore Fallimentare** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Curatore Fallimentare** (se smarrito)
* **DELEGA\_CONSEGNA\_VEICOLO\_FALLIMENTO** in originale compilata e firmata dal **Delegante** (**Curatore Fallimentare**) e **Delegato** (Se si delega qualcun altro)
* Denuncia di smarrimento originale Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Delegato** (se smarrito)
* **DICHIARAZIONE\_SOSTITUTIVA\_CURATORE\_FALLIMENTARE** in originale compilata e firmata









**6)DEMOLIZIONE MEZZO INTESTATO AD ASSOCIAZIONE:**

**Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo (può fare foto direttamente dal telefono o caricamento file):**

* **Visura Associazione (se riconosciuta, dove si vedono i dati del Presidente)** o **Atto Costitutivo, Statuto e certificato di attribuzione Codice Fiscale dell'ente (se associazione non riconosciuta).**
* Libretto Circolazione (fronte e retro)
* Denuncia di smarrimento Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo (fronte e retro) (Se è Digitale non deve caricarlo e non serve)
* Denuncia di smarrimento Certificato di Proprietà Cartaceo (se smarrito)
* Carta Identità **Presidente Associazione** (fronte e retro) o Patente di guida **Presidente Associazione** (fronte e retro) se Carta Identità **Presidente Associazione** non disponibile
* Codice Fiscale **Presidente Associazione** (fronte e retro)
* Denuncia di smarrimento Carta Identità **Presidente Associazione** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Presidente Associazione** (se smarrito)



Se **Presidente Associazione** delega un'altra persona alla consegna del mezzo al Demolitore:

* Carta Identità **Delegato** (fronte e retro) o Patente di guida **Delegato** (fronte e retro) se Carta Identità **Delegato** non disponibile
* Codice Fiscale (fronte e retro) **Delegato**
* Denuncia di smarrimento Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Delegato** (se smarrito)



**Documenti PDF da stampare e firmare prima del ritiro:**

* **DELEGA\_CONSEGNA\_VEICOLO\_ASSOCIAZIONE** (auto compilata dal sistema) se **Presidente Associazione** delega un'altra persona alla consegna del mezzo al Demolitore
* **DICHIARAZIONE\_SOSTITUTIVA\_PRESIDENTE\_ASSOCIAZIONE** (auto compilata dal sistema)



**Documenti da consegnare al Demolitore al ritiro (nella pagina del cliente ci dovrà essere una sezione riepilogativa. Dovrà ricevere anche un sms, email e notifica APP come promemoria):**

* Libretto di Circolazione originale
* Denuncia di smarrimento originale Libretto di Circolazione (se smarrito)
* Certificato di Proprietà Cartaceo originale (se è Digitale no)
* Denuncia di smarrimento originale Certificato di Proprietà Cartaceo (se smarrito)
* Denuncia di smarrimento originale Carta Identità **Presidente Associazione** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Presidente Associazione** (se smarrito)
* **DELEGA\_CONSEGNA\_VEICOLO\_ASSOCIAZIONE** in originale compilata e firmata dal **Delegante** (**Presidente Associazione**) e **Delegato** (Se si delega qualcun altro)
* Denuncia di smarrimento originale Carta Identità **Delegato** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Delegato** (se smarrito)
* **DICHIARAZIONE\_SOSTITUTIVA\_PRESIDENTE\_ASSOCIAZIONE** in originale compilata e firmata









**7)DEMOLIZIONE MEZZO PROPRIETARIO NON INTESTATARIO PRA:**

**Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo (può fare foto direttamente dal telefono o caricamento file):**

* Libretto Circolazione (fronte e retro) (**Se non ha questo documento non può procedere alla Demolizione**)
* Certificato di Proprietà Cartaceo (fronte e retro) (**Se non ha questo documento non può procedere alla Demolizione**) (Se è Digitale non deve caricarlo e non serve)
* Carta Identità **Proprietario non Intestatario** (fronte e retro) o Patente di guida **Proprietario non Intestatario** (fronte e retro) se Carta Identità **Proprietario non Intestatario** non disponibile
* Codice Fiscale **Proprietario non Intestatario** (fronte e retro)
* Denuncia di smarrimento Carta Identità **Proprietario non Intestatario** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Proprietario non Intestatario** (se smarrito)



**Documenti PDF da stampare e firmare prima del ritiro:**

* **DICHIARAZIONE\_SOSTITUTIVA\_RADIAZIONE\_PROPRIETARIO\_NON\_INTESTATARIO** (auto compilata dal sistema)



**Documenti da consegnare al Demolitore al ritiro (nella pagina del cliente ci dovrà essere una sezione riepilogativa. Dovrà ricevere anche un sms, email e notifica APP come promemoria):**

* Libretto di Circolazione originale
* Certificato di Proprietà Cartaceo originale (se è Digitale no)
* Denuncia di smarrimento originale Carta Identità **Proprietario non Intestatario** (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale **Proprietario non Intestatario** (se smarrito)
* **DICHIARAZIONE\_SOSTITUTIVA\_RADIAZIONE\_PROPRIETARIO\_NON\_INTESTATARIO** in originale compilata e firmata

**NOTA: In questa casistica non è possibile delegare qualcun altro alla consegna del mezzo da demolire**





**8) DEMOLIZIONE MEZZO CON TARGHE STRANIERE (NON SI APPLICA INTEGRAZIONE 1 E 2)**

Cosa succede al momento del ritiro:

Il demolitore prenderà in carico il veicolo, rimuoverà le targhe straniere e le riconsegnerà subito a te. Contestualmente, ti rilascerà il Certificato di Rottamazione.

Per completare la pratica, dovrai portare sia le targhe che il certificato di rottamazione nel Paese d'origine del veicolo (presso le autorità competenti, la motorizzazione locale o il Consolato in Italia) per richiedere la cancellazione definitiva e la cessazione del bollo/tasse estere.



**Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo (può fare foto direttamente dal telefono o caricamento file):**



* Libretto di Circolazione **Estero** (fronte e retro)
* Denuncia di smarrimento Libretto di Circolazione **Estero** (se smarrito)
* Carta Identità **Proprietario mezzo** (fronte e retro) o Patente di guida **Proprietario mezzo** (fronte e retro) se Carta Identità **Proprietario mezzo** non disponibile
* Codice Fiscale **Proprietario mezzo** (fronte e retro)
* Denuncia di smarrimento Carta Identità **Proprietario mezzo** (se smarrita)
* Denuncia di smarrimento Codice Fiscale **Proprietario mezzo** (se smarrito)



**Documenti da consegnare al Demolitore al ritiro (nella pagina del cliente ci dovrà essere una sezione riepilogativa. Dovrà ricevere anche un sms, email e notifica della piattaforma come promemoria):**

* Libretto di Circolazione **Estero** in **ORIGINALE**
* Denuncia di smarrimento originale Libretto di Circolazione **Estero** (se smarrito)
* Targhe del veicolo (presenti sul mezzo al momento del ritiro)
* Denuncia di smarrimento originale Carta Identità Proprietario mezzo (se smarrita)
* Denuncia di smarrimento originale Codice Fiscale Proprietario mezzo (se smarrito)



**NOTA: In questa casistica non è possibile delegare qualcun altro alla consegna del mezzo da demolire.**











**INTEGRAZIONE 1:**

**MEZZO CON FERMO AMMINISTRATIVO**



**NOTE PER L'UTENTE** (Visualizzate nell'interfaccia dell'App)

Quando usare questa opzione:

Questa voce si attiva automaticamente se risponde "SÌ" alla domanda "Sul **mezzo** è presente un uno o più fermi amministrativi?".



**Attenzione:** (aggiornato alla **Legge 26 gennaio 2026, n. 14**, in vigore dal 20/02/2026: il fermo NON blocca più la radiazione per demolizione — resta esclusa solo l'esportazione all'estero)

Se sul **mezzo** c'è un fermo amministrativo servono DUE cose:

1. La **dichiarazione di stato del veicolo con fermo amministrativo e richiesta di demolizione**: la troverai già compilata nella tua Area Personale, pronta da scaricare, stampare e firmare.

2. L'**Attestazione di inutilizzabilità del veicolo**, rilasciata dal **Comune o dalla Polizia locale** (servizio a pagamento: costi e tempi variano da Comune a Comune). Va caricata nell'Area Personale e consegnata in **originale** al ritiro: senza, la radiazione al PRA non si può fare.



Ricorda che la demolizione serve a eliminare il veicolo, ma il debito originale che ha causato il fermo non si cancella e rimarrà legato al codice fiscale del proprietario.

**Ricorda che la demolizione serve per togliere il veicolo dalla circolazione, ma il debito originale che ha causato il fermo non si cancella con la rottamazione dell'auto e rimarrà legato al codice fiscale del proprietario.**



**Regola Universale per Generazione PDF e Firme:**

Il sistema deve generare il documento PDF denominato **DICHIARAZIONE\_SOSTITUTIVA\_STATO\_VEICOLO\_CON\_FERMO\_AMMINISTRATIVO**

I campi anagrafici del dichiarante e la firma sul modulo del fermo devono essere associati automaticamente dal Backend al medesimo soggetto definito nella casistica principale inserita dall'utente.

Nello specifico:



Se applicato a **Casistica 1** Il modulo viene intestato e firmato da: **Proprietario mezzo**

Se applicato a **Casistica 2** o **Casistica 3** Il modulo viene intestato e firmato da: **Erede o Eredi** (esclusivamente quelli identificati come **"Accettanti"**)

Se applicato a **Casistica 4** Il modulo viene intestato e firmato da: **Legale Rappresentante Società/Azienda**

Se applicato a **Casistica 5** Il modulo viene intestato e firmato da: **Curatore Fallimentare**

Se applicato a **Casistica 6** Il modulo viene intestato e firmato da: **Presidente Associazione**

Se applicato a **Casistica 7** Il modulo viene intestato e firmato da: **Proprietario non Intestatario**



**Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo:**

(Il sistema richiede i documenti già previsti dalla casistica principale **da 1 a 7** selezionata dall'utente, senza aggiungere slot duplicati, PIÙ:)

* **Attestazione di inutilizzabilità del veicolo** (rilasciata dal Comune o dalla Polizia locale — Legge 26 gennaio 2026, n. 14)



**Documenti PDF da stampare e firmare prima del ritiro (si aggiunge all'elenco della casistica principale):**

**DICHIARAZIONE\_SOSTITUTIVA\_STATO\_VEICOLO\_CON\_FERMO\_AMMINISTRATIVO** (auto compilata dal sistema con i dati della persona che firma la pratica principale)



Documenti da consegnare al Demolitore al ritiro (da aggiungere nella sezione riepilogativa della pagina cliente, sms, email e notifica APP come promemoria insieme ai documenti della casistica principale):

* **DICHIARAZIONE\_SOSTITUTIVA\_STATO\_VEICOLO\_CON\_FERMO\_AMMINISTRATIVO** in originale compilata e firmata dal soggetto attivo (**Proprietario**, **Legale Rappresentante**, **Curatore**, **Presidente, Eredi Accettanti, Proprietario non Intestatario**) — dal 10/07/2026 il titolo del modulo è "Dichiarazione di stato del veicolo con fermo amministrativo e richiesta di demolizione" (adeguato alla L. 14/2026), una sola firma
* **Attestazione di inutilizzabilità del veicolo in ORIGINALE** (rilasciata dal Comune o dalla Polizia locale — Legge 26 gennaio 2026, n. 14)



&#x20;

**INTEGRAZIONE 2:**

**TARGA O TARGHE SMARRITE**
Nel flusso di compilazione del modulo inserire la domanda obbligatoria:

**"Le targhe sono presenti?"**



Documenti che deve caricare in NoiDemoliamo Area personale dopo la compilazione del modulo (può fare foto direttamente dal telefono o caricamento file):

* **Denuncia di smarrimento targa o targhe del mezzo (se smarrita/e)**



Documenti da consegnare al Demolitore al ritiro (nella pagina del cliente ci dovrà essere una sezione riepilogativa. Dovrà ricevere anche un sms, email e notifica APP come promemoria):

Targa o targhe rimaste (se ne manca una sola)

* **Denuncia di smarrimento originale targa o targhe del mezzo (se smarrita/e)**







**INTEGRAZIONE 3:**
Possono esserci in futuro altre casistiche da inserire

