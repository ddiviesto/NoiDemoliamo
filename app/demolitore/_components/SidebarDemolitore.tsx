...
function Voce({ attiva, disabilitata, onClick, icona, label, extra, espansa }: {
  attiva?: boolean
  ...
  return (
    <button
      ...
      style={{
        ...
        color: disabilitata ? '#B6BEC9' : attiva ? '#1D4ED8' : '#5B6779',
        background: attiva ? '#EFF4FF' : 'transparent',
        cursor: disabilitata ? 'default' : 'pointer',
      }}
      onMouseEnter={e => { if (!attiva && !disabilitata) e.currentTarget.style.background = '#F4F6F9' }}
...
function VociMenu({ attiva, espansa, nome, onPratiche, onAzienda, onEsci }: {
  ...
  return (
    <>
      {/* In testa c'è IL NOME DEL DEMOLITORE (richiesta Davide 23/07) */}
      <div className="flex items-center gap-3 whitespace-nowrap" style={{ padding: '4px 10px 13px', borderBottom: '1px solid #F0F2F5', marginBottom: 8 }}>
        <span className="flex items-center justify-center flex-shrink-0" style={{ width: 28, height: 28, borderRadius: 8, background: '#2563eb', color: '#fff', fontWeight: 800, fontSize: 13 }}>N</span>
        <span className="transition-opacity duration-150 truncate" style={{ fontSize: 12.5, fontWeight: 800, color: '#111827', opacity: espansa ? 1 : 0, maxWidth: 150 }}>{nome || '…'}</span>
      </div>
      ...
      <Voce disabilitata espansa={espansa} label="Fatturazione"
        extra={<span style={{ fontSize: 8.5, fontWeight: 700, background: '#F0F2F5', color: '#9AA3AF', borderRadius: 999, padding: '1px 7px' }}>PRESTO</span>}
      ...
      <div className="mt-auto" style={{ borderTop: '1px solid #F0F2F5', paddingTop: 8 }}>
...
export default function SidebarDemolitore({ attiva, apertaMobile, onChiudiMobile, onPratiche, onAzienda, onEsci }: {
  ...
  return (
    <>
      ...
      <div
        className="hidden lg:flex flex-col flex-shrink-0 sticky top-0 h-screen bg-white overflow-hidden"
        style={{
          width: hover ? 212 : 58,
          borderRight: '1px solid #E5E7EB',
          transition: 'width 0.22s ease',
          boxShadow: hover ? '6px 0 24px rgba(15,27,51,0.08)' : 'none',
          padding: '12px 8px',
          zIndex: 30,
        }}
      ...
      />
      <div
        className="lg:hidden fixed top-0 left-0 bottom-0 z-50 bg-white flex flex-col shadow-2xl transition-transform duration-300"
        style={{ width: 240, padding: '14px 10px', transform: apertaMobile ? 'translateX(0)' : 'translateX(-105%)' }}
      >
